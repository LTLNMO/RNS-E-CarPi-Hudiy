...
def set_thread_priority():
    """Attempt to set real-time thread priority for the TX thread."""
    try:
        import ctypes
        SCHED_FIFO = 1
        ...
        ctypes.CDLL("libpthread.so.0").pthread_setschedparam(
            ctypes.CDLL("libpthread.so.0").pthread_self(), SCHED_FIFO, ctypes.byref(param)
        )
    except Exception:
        pass
...
class TextScroller:
    ...
    def _update_bytes(self):
        # Short text u2014 center in the 8-char window
        # Long text u2014 left-align the current scroll window
        if len(self.raw_text) <= self.width:
...
class LineController:
    ...
    def trigger_slam(self, payload: bytes):
        """Called by CANWatcher u2014 non-blocking, posts to slam queue."""
        self._slam_q.put(payload)

    def force_refresh(self):
        """Force the baseline refresh to fire on the next TX loop iteration."""
    ...
    def run(self):
        """TX thread u2014 handles slam queue, tail schedule, and baseline refresh."""
        set_thread_priority()
        next_refresh = 0.0
        ...
        while True:
            ...
            # --- Drain slam queue u2014 keep only the most recent payload ---
            payload = None
            ...
            if payload is not None:
                if self._watcher and self._watcher.tv_active:
            ...
            # --- Tail sends u2014 always use live scroller bytes ---
            with self._tail_lock:
...
class CANWatcher:
    ...
    def run(self):
        """RX thread u2014 never sleeps, never blocks on slam sends."""
        while True:
            try:
                ...
                if cid == self.id_source and len(msg.data) >= 4:
                    ...
                    if self.tv_active and not was_active:
                        # Reset stale state from previous ignition cycle
                        if self.dis_ctrl:
                            self.dis_ctrl._last_content_time = time.time()
                            self.dis_ctrl._no_media_shown    = False
                        if self.ctrl_l1:
                            self.ctrl_l1.last_sent = None
                            self.ctrl_l1.force_refresh()
                        if self.ctrl_l2:
                            self.ctrl_l2.last_sent = None
                            self.ctrl_l2.force_refresh()
                ...
                time.sleep(0.1)  # Brief pause before retrying on CAN error
...
class DISController:
    """
    Thin orchestrator u2014 loads config, wires components together, runs main loop.
    """
    ...
    def __init__(self):
        ...
        self._sub.setsockopt(zmq.RCVTIMEO, 1000)  # 1s timeout so shutdown is clean
        ...
        # Boot timing and silence timeout
        self._boot_no_media_delay = float(feat.get("boot_no_media_delay", 3.0))
        self._no_source_delay     = float(feat.get("no_source_delay", 30.0))
        self._boot_time           = time.time()
        self._hudiy_connected     = False
        self._last_content_time   = 0.0
    ...
    def _show_no_media(self):
        """Write No Media to both lines."""
        if self._ctrl_l1:
            if self.NO_MEDIA_L1:
                self._ctrl_l1.scroller.set_text(self.NO_MEDIA_L1)
            else:
                self._ctrl_l1.scroller.clear()
        if self._ctrl_l2:
            if self.NO_MEDIA_L2:
                self._ctrl_l2.scroller.set_text(self.NO_MEDIA_L2)
            else:
                self._ctrl_l2.scroller.clear()
    ...
    def _metadata_listener(self):
        while self.running:
            try:
                ...
                self._last_source_id    = p.get("source_id", 0)
                self._no_media_shown    = False
                if self._ctrl_l1:
                    self._ctrl_l1.scroller.set_text(l1_text if l1_text else self.NO_MEDIA_L1)
                    self._ctrl_l1.force_refresh()
                if self._ctrl_l2:
                    self._ctrl_l2.scroller.set_text(l2_text if l2_text else self.NO_MEDIA_L2)
                    self._ctrl_l2.force_refresh()
            except zmq.Again:
                pass  # Timeout u2014 check self.running and loop
            except Exception:
                time.sleep(0.05)
    ...
    def run(self):
        ...
        while self.running:
            ...
            since_boot    = now - self._boot_time
            since_content = now - self._last_content_time if self._last_content_time else float("inf")

            hudiy_not_yet = not self._hudiy_connected and since_boot    >= self._boot_no_media_delay
            # Only timeout if hudiy silent AND no active source (source_id=0 means no source)
            hudiy_silent  = self._hudiy_connected     and since_content >= self._no_source_delay \
                            and self._last_source_id == 0

            if not self._no_media_shown and (hudiy_not_yet or hudiy_silent):
                self._show_no_media()
